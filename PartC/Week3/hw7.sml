...
fun preprocess_prog (e) =
	...
	LineSegment(x1, y1, x2, y2) =>
		...
		then Point((x1 + x2) / 2.0, (y1 + y2) / 2.0)
		else if real_close (x1, x2)
		then if y1 < y2 then LineSegment(x1, y1, x2, y2) else LineSegment(x2, y2, x1, y1)
		else if x1 < x2 then LineSegment(x1, y1, x2, y2) else LineSegment(x2, y2, x1, y1)
